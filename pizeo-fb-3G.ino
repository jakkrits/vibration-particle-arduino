...
void publishData() {
    ...
    // double value y is a cosine, so the values will rise and fall nicely over 360 steps
    double y = cos((double)(x % 360) * M_PI / 180.0);

    // z is a random integer
    int z = rand();
    ...
    char buf[256];
    snprintf(buf, sizeof(buf), "{\"x\":%d,\"y\":%.3f,\"z\":%d,\"total\":%d,\"n\":\"%s\"}", x, y, z, total, retainedData.deviceName);
